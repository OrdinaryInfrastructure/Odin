...
public interface IRabbitConnectionService
{
    ...
    /// resubscription may be attempted.
    /// Throws if the initial connection, channel and consumer cannot be established.
    /// </summary>
    /// <param name="queueName"></param>
    /// <param name="onFailure">Called if the connection breaks. Exceptions thrown by the given callback are ignored. May be called multiple times for the same failure.</param>
    /// <param name="onConsume">Called for each consumed message. Exceptions thrown by the given callback are ignored.</param>
    ...
    /// <returns>An async callback that, when invoked, cancels the subscription. When this is done, the onFailure callback is called twice, which should be ignored.</returns>
    public Task<Func<Task>> SubscribeToConsume(string queueName, Func<Exception, Task> onFailure, Func<ConsumedMessage, Task> onConsume, bool autoAck, ushort prefetchCount = 200, TimeSpan? channelCheckPeriod = null);
}